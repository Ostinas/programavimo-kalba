...
class ForNode:
    def __init__(self, var_namme_tok, start_value_node, end_value_node, step_value_node, body_node):
...
class Parser:
    ...
    def atom(self):
        ...
        if tok.type == TT_STRING:
            ...
            list_exp = res.register(self.list_exp())
            if res.error: return res
            ...
            for_exp = res.register(self.for_exp())
            if res.error: return res
            return res.success(for_exp)

        elif tok.matches(TT_KEYWORD, 'WHILE'):
            while_exp = res.register(self.while_exp())
            if res.error: return res
    ...
    def comp_exp(self):
        ...
        node = res.register(self.bin_op(self.arith_exp, (TT_EE, TT_NE, TT_LT, TT_GT, TT_LTE, TT_GTE)))
    ...
    def func_def(self):
        ...
        if self.current_tok.type != TT_ARROW:
            return res.failue(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Expected '->'"
            ))
    ...
    def execute_pop(self, exec_ctx):
        ...
        try:
            ...
            return RTResult().failure(RTError(self.pos_start, self.pos_end,
             "Element at this index could not be removed from list because index is out of bounds", exec_ctx))
...
def run(fn, text):
    ...
    if syntaxTree.error: return None, syntaxTree.error

    interpreter = Interpreter()
    context = Context('<program>')
    context.symbol_table = global_symbol_table
    result = interpreter.visit(syntaxTree.node, context)
